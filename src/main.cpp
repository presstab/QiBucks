...
const string strMessageMagic = "Qibuck Signed Message:\n";
...
{
    if (pindexBest->nHeight > (YEARLY_BLOCKCOUNT*37)) // 32 years.
        return nFees; // We need a better way of doing this instead of blockcount.

    int64_t nRewardCoinYear;

    nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE;
    if (pindexBest->nHeight > (5*YEARLY_BLOCKCOUNT)) // After 5 years, 2.5% PoS
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Qibuck", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Qibuck 2014";
        CTransaction txNew;
        txNew.nTime = 1405991117;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1405991117;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1697490;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xd0ef9a2bf5693fddf39cf5945665a6d5f5a80944f9379d85a5305f1b4cf722dc"));
        block.print();
